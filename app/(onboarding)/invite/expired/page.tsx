...
export default async function InviteExpiredPage({ searchParams }: Props) {
  const { reason } = await searchParams;
  const isConsumed = reason === "consumed";
          ...
          {isConsumed ? (
            ...
            <>
              ...
              <p className="text-sm text-muted-foreground mb-8 max-w-xs">
                ...
                Please log in with the email and password you created.
              </p>
              <Link
                href="/auth/login"
                className="w-full h-13 flex items-center justify-center gap-2 bg-primary text-white rounded-xl font-medium text-sm hover:bg-primary/90 transition-colors px-4 py-3"
              >
                Go to Login
              </Link>
            </>
            ...
            <>
              ...
              <p className="text-sm text-muted-foreground mb-8 max-w-xs">
                ...
                Please contact your organisation to request a new invitation.
              </p>
              <Link
                href="/auth/login"
